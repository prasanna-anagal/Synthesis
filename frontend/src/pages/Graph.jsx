...
import { Loader2, RefreshCw, Network, ZoomIn, ZoomOut, Maximize2, X } from 'lucide-react'
...
export default function Graph() {
  ...
  if (error) return (
    <div className="p-10 text-center text-slate-600">
      ...
      <button type="button" onClick={() => loadGraph(true)} className="px-4 py-2 rounded-lg bg-indigo-600 text-white font-semibold text-xs cursor-pointer">
        Retry
      </button>
    </div>
  )
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-slate-50/30">
      {/* Header */}
      <div className="px-6 py-4 border-b border-slate-200/80 bg-white flex items-center justify-between flex-shrink-0 z-10">
        <div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 bg-white text-xs font-semibold text-slate-700 hover:bg-slate-50 cursor-pointer"
          >
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        {graphData?.nodes?.length > 0 && (
          <div className="absolute bottom-6 right-6 flex gap-1 bg-white border border-slate-200/90 rounded-xl p-1 shadow-md z-20">
            <button type="button" onClick={() => handleZoom(1.3)} title="Zoom in" className="p-2 text-slate-600 hover:bg-slate-100 rounded-lg cursor-pointer"><ZoomIn className="w-4 h-4" /></button>
            <button type="button" onClick={() => handleZoom(0.7)} title="Zoom out" className="p-2 text-slate-600 hover:bg-slate-100 rounded-lg cursor-pointer"><ZoomOut className="w-4 h-4" /></button>
            <button type="button" onClick={() => handleZoom(0)} title="Reset zoom" className="p-2 text-slate-600 hover:bg-slate-100 rounded-lg cursor-pointer"><Maximize2 className="w-4 h-4" /></button>
          </div>
        )}

        {/* Concept Inspector Overlay */}
        {selectedNode && (
          <div className="absolute top-4 right-4 w-64 bg-white rounded-2xl border border-slate-200 p-5 shadow-xl z-20">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-indigo-600" />
                <h3 className="font-bold text-sm text-slate-900">{selectedNode.label}</h3>
              </div>
              <button type="button" onClick={() => setSelectedNode(null)} className="text-slate-400 hover:text-slate-700">
                <X className="w-4 h-4" />
              </button>
            </div>
